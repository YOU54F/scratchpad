...
const runBingo = (bingoNumbers: string[], boards: string[][][]) => {
  ...
  bingoNumbers.every((bingoNumber) => {
    markBoards(bingoNumber, boards);
    const winningBoard = findWinningBoard(boards);
    if (winningBoard && winningBoard !== -1) {
      results.winner = winningBoard;
      results.lastCalledNumber = bingoNumber;
      results.score = calculateScore(
        results.lastCalledNumber,
        boards[results.winner]
      );
      return;
    }
    return true;
...
const calculateScore = (multiplier: string, board: string[][]) => {
  let totaliser = 0;
  board.map((r) =>
    r.map((cell) => (cell !== "X" ? (totaliser += parseFloat(cell)) : "0"))
  );
  const score = parseFloat(multiplier) * totaliser;
  return score;
};
